...
describe("GridStackRenderProvider", () => {
  ...
  describe("Options Comparison Logic", () => {
    it("should compare options excluding children property", () => {
      // Test the logic used in compareOptionsWithoutChildren
      const options1 = {
        column: 12,
        margin: 10,
        children: [{ id: "1", w: 1, h: 1 }],
      }

      const options2 = {
        column: 12,
        margin: 10,
        children: [{ id: "2", w: 2, h: 2 }],
      }

      // When excluding children, these should be equal
      const { children: _, ...opts1 } = options1
      const { children: __, ...opts2 } = options2

      expect(JSON.stringify(opts1)).toBe(JSON.stringify(opts2))
      expect(opts1.column).toBe(opts2.column)
      expect(opts1.margin).toBe(opts2.margin)
    })

    it("should detect differences in options excluding children", () => {
      const options1 = {
        column: 12,
        margin: 10,
        children: [{ id: "1", w: 1, h: 1 }],
      ...
      expect(opts1.column).not.toBe(opts2.column)
    })
  })
})
